document.addEventListener("DOMContentLoaded", function() {
    ...
    function updateNumbering() {
        var numberedObjects = objectsOnCanvas.filter(obj => !obj.image.alt.includes("ob-6"));
        
        // Update object numbering for numbered objects only
        numberedObjects.forEach(function(obj, index) {
            obj.number = index + 1; // Start numbering from 1
        });
    
        // Set the object counter based on the total number of numbered objects
        objectCounter = numberedObjects.length + 1;
    }
    ...
    function loadCanvasData(canvasData) {
        ...
        parsedData.forEach(function(objData) {
            ...
            if (!imgElement.alt.includes("ob-6")) {
                newObj.number = objData.number || objectCounter++; // Assign a new number if needed
            }
